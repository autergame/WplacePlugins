...
(async function () {
  ...
  if (!mapKey) {
    ...
    if (mapKey) {
      ...
      const useDefaultKey = window.confirm(
    ...
    if (configArg?.url && configArg?.url === originalThemeUrl) {
      ...
      if (selectedTheme.mapTiler) {
        if (customColor.background) {
          const backgroundIndex = layersMod.findIndex(
            (layer) => layer.id === "Background"
          );
          layersMod[backgroundIndex].paint["background-color"] =
            customColor.background;
        }
        if (customColor.water) {
          const waterIndex = layersMod.findIndex(
  ...
  function putUI(selector) {
    let menuItemsHTML = Object.entries(themesMap)
      .map(([id, theme]) => {
    ...
    menuItemsHTML += `
                <div class="flex items-center justify-between" style="padding-block: .375rem; padding-inline: .75rem;">
                    <span>No Label / Text</span><div class="flex items-center gap-2">
                        <div class="tooltip">
                            <input id="no_label" type="checkbox" ${
                              customNoLabel ? "checked" : ""
                            } class="btn btn-sm btn-circle" onchange="window.changeNoLabel(event);">
                        </div>
                    </div>
                </div>
            `;
    menuItemsHTML += `
                <div class="flex items-center justify-between" style="padding-block: .375rem; padding-inline: .75rem;">
                    <span>Backgroud Color</span><div class="flex items-center gap-2">
                        <div class="tooltip">
                            <input id="backgroud_color" type="color" value="${
                              customColor.background || "#000000"
                            }" class="btn btn-sm btn-circle" onchange="window.changeColor('Background', event);">
                        </div>
                    </div>
                </div>
            `;
    menuItemsHTML += `
                <div class="flex items-center justify-between" style="padding-block: .375rem; padding-inline: .75rem;">
                    <span>Water Color</span><div class="flex items-center gap-2">
                        <div class="tooltip">
                            <input id="water_color" type="color" value="${
                              customColor.water || "#000000"
                            }" class="btn btn-sm btn-circle" onchange="window.changeColor('Water', event);">
                        </div>
                    </div>
                </div>
            `;
  ...
  const observer = new MutationObserver((mutations, observer) => {
    for (const mutation of mutations) {
      ...
      const selector = mutation.target.querySelector(
        "div.flex.flex-col.items-center.gap-3"
      );
      if (!selector.querySelector(".dropdown.dropdown-end #map-theme-btn")) {
        putUI(selector);
        observer.disconnect();
      }
    }
  });
  const leftButtons = await waitForElement(
    "body div.absolute.right-2.top-2.z-30"
  );
  ...
  setTimeout(() => {
    ...
    if (
      selector &&
      !selector.querySelector(".dropdown.dropdown-end #map-theme-btn")
    ) {
      putUI(selector);
      observer.disconnect();
    }
